...
stdenv.mkDerivation rec {
  ...
  buildInputs = [ elfutils zlib bzip2 lzma ncurses ];
  nativeBuildInputs = [ wget texinfo6 bison ];

  makeFlags = [ "LINKTYPE=static" "DESTDIR=$out" "PREFIX=$out" "DESTDIR=$out" "V=1" ];

  postPatch = ''
    substituteInPlace Makefile --replace /usr/bin/wget ${wget}/bin/wget
    substituteInPlace Makefile --replace '/usr/bin/' ""
